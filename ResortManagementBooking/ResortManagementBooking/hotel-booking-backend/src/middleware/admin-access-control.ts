 ...
 */
export const restrictAdminToSubdirectory = (req: Request, res: Response, next: NextFunction) => {
  // Get the origin from the request
  const origin = req.headers.origin || "";
  const referer = req.headers.referer || "";
  
  // Allow admin login from admin-login path regardless of origin
  let isFromAdminLogin = 
    referer.includes("/admin-login") ||
    req.path.includes("/admin-login");
  
  // Safely check referer pathname if referer exists and is valid
  if (referer) {
    try {
      const refererUrl = new URL(referer);
      isFromAdminLogin = isFromAdminLogin || refererUrl.pathname === "/admin-login";
    } catch (e) {
      // Invalid referer URL - ignore silently
    }
  }
  
  // Check if request is coming from admin subdirectory or admin portal
  const isFromAdminSubdirectory = 
    isFromAdminLogin ||
  ...
  const isFromMainWebsite = 
    origin.includes(":5174") || // Main frontend dev
    (process.env.FRONTEND_URL && origin === process.env.FRONTEND_URL);
  
  // Store the origin type for later use in auth routes
  (req as any).originType = isFromAdminSubdirectory ? "admin" : isFromMainWebsite ? "main" : "unknown";
 ...
 */
export const blockAdminFromMainSite = (req: Request, res: Response, next: NextFunction) => {
