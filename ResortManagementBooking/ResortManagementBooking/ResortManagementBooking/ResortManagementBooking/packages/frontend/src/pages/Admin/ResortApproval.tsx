...
const ResortApproval: React.FC = () => {
  ...
  const { data: pendingData, isLoading: pendingLoading } = useQuery(
    ...
    () => apiClient.fetchPendingResorts(currentPage, 10),
    {
  ...
  const { data: allData, isLoading: allLoading } = useQuery(
    ...
    () => apiClient.fetchAllResortsForApproval(
      currentPage, 
      10, 
  ...
  const { data: stats } = useQuery("approvalStats", apiClient.fetchApprovalStats);

  const approveMutation = useMutation(apiClient.approveResort, {
    onSuccess: (data) => {
      toast({
  ...
  const rejectMutation = useMutation(
    ({ resortId, reason }: { resortId: string; reason: string }) =>
      apiClient.rejectResort(resortId, reason),
    {
      onSuccess: (data) => {
        toast({
