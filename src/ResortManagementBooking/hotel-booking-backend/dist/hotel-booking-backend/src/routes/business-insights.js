...
const getDashboardData = () => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const recentBookings = allBookings.filter((booking) => new Date(booking.createdAt) >= thirtyDaysAgo).length;
    ...
    const previousMonthRevenue = allBookings
        .filter((booking) => {
        const bookingDate = new Date(booking.createdAt);
        return (bookingDate >= new Date(now.getFullYear(), now.getMonth() - 1, 1) &&
    ...
    if (popularDestinations.length === 0) {
        ...
        const hotelDestinations = hotels.reduce((acc, hotel) => {
            if (acc[hotel.city]) {
        ...
        const fallbackDestinations = Object.values(hotelDestinations)
            .map((dest) => ({ _id: dest._id, count: dest.count, totalRevenue: dest.totalRevenue, avgPrice: dest.avgPrice }))
            .sort((a, b) => b.count - a.count)
            .slice(0, 5);
        popularDestinations.push(...fallbackDestinations);
    }
    const bookingDates = allBookings.reduce((acc, booking) => {
    ...
    const hotelPerformance = yield booking_1.default.aggregate([
...
router.get("/dashboard/public", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const businessInsightsData = yield getDashboardData();
...
const getForecastData = () => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const allBookings = yield booking_1.default.find();
    const historicalBookings = allBookings.filter((booking) => new Date(booking.createdAt) >= twoMonthsAgo);
    const weekGroups = historicalBookings.reduce((acc, booking) => {
        const bookingDate = new Date(booking.createdAt);
        ...
        if (!acc[weekKey])
            acc[weekKey] = { week: weekKey, bookings: 0, revenue: 0 };
        acc[weekKey].bookings++;
        acc[weekKey].revenue += booking.totalCost;
        return acc;
    }, {});
    let weeklyData = Object.values(weekGroups)
        .map((week) => ({ week: week.week, bookings: week.bookings, revenue: Math.round(week.revenue * 100) / 100 }))
    ...
    if (lastMonthBookings > 0)
        seasonalGrowth = ((currentMonthBookings - lastMonthBookings) / lastMonthBookings) * 100;
    return {
...
router.get("/forecast/public", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const forecastData = yield getForecastData();
...
router.get("/forecast", auth_1.default, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const forecastData = yield getForecastData();
        res.status(200).json(forecastData);
    }
...
const getSystemStatsData = () => __awaiter(void 0, void 0, void 0, function* () {
    const memUsage = process.memoryUsage();
    const cpuUsage = process.cpuUsage();
    const allHotels = yield hotel_1.default.find();
    const allBookings = yield booking_1.default.find();
    const totalBookings = allBookings.length;
    const totalRevenue = allBookings.reduce((sum, booking) => sum + (booking.totalCost || 0), 0);
...
router.get("/system-stats/public", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const performanceData = yield getSystemStatsData();
...
router.get("/system-stats", auth_1.default, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const performanceData = yield getSystemStatsData();
        res.status(200).json(performanceData);
    }
