...
const CottageSchema = new mongoose_1.default.Schema({
    id: { type: String, required: true },
    name: { type: String, required: true },
    type: { type: String, required: true },
...
hotelSchema.index({ isApproved: 1, lastUpdated: -1 }); // Fix for homepage hotels timeout
// Additional required indexes
hotelSchema.index({ userId: 1 }); // ownerId is stored as userId field
hotelSchema.index({ status: 1 });
hotelSchema.index({ isApproved: 1 }); // Single field index for approval queries
