          ...
          if (Array.isArray(updateData[field])) {
            updateData[field] = updateData[field].map((item: any) => {
              ...
              if (field === 'rooms') {
                ...
                // Process includedEntranceFee for rooms
                if (processedItem.includedEntranceFee) {
                  console.log(`=== ROOMS JSON INCLUDED ENTRANCE FEE DEBUG ===`);
                  console.log(`Room includedEntranceFee before processing:`, processedItem.includedEntranceFee);
                  
                  processedItem.includedEntranceFee = {
                    enabled: processedItem.includedEntranceFee.enabled === true || processedItem.includedEntranceFee.enabled === 'true',
                    adultCount: Number(processedItem.includedEntranceFee.adultCount) || 0,
                    childCount: Number(processedItem.includedEntranceFee.childCount) || 0,
                  };
                  
                  console.log(`Room includedEntranceFee after processing:`, processedItem.includedEntranceFee);
                }
              }
              ...
              if (field === 'cottages') {
                ...
                // Process includedEntranceFee for cottages
                if (processedItem.includedEntranceFee) {
                  console.log(`=== COTTAGES JSON INCLUDED ENTRANCE FEE DEBUG ===`);
                  console.log(`Cottage includedEntranceFee before processing:`, processedItem.includedEntranceFee);
                  ...
                  console.log(`Cottage includedEntranceFee after processing:`, processedItem.includedEntranceFee);
                }
              }
...
router.put(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      console.log("=== ALL INCLUDED ENTRANCE FEE KEYS RECEIVED ===");
      console.log("includedEntranceFee keys:", includedEntranceFeeKeys);
      includedEntranceFeeKeys.forEach(key => {
      ...
      while (req.body[`rooms[${createRoomIndex}][id]`]) {
        ...
        console.log(`Room ${createRoomIndex} childCount:`, req.body[`rooms[${createRoomIndex}][includedEntranceFee][childCount]`]);
        
        // Check all keys that contain includedEntranceFee for this room
      ...
      while (req.body[`cottages[${updateCottageIndex}][id]`]) {
        ...
        // Parse includedEntranceFee for cottages
        console.log(`=== COTTAGES INCLUDED ENTRANCE FEE DEBUG ===`);
        console.log(`Cottage ${updateCottageIndex} enabled:`, req.body[`cottages[${updateCottageIndex}][includedEntranceFee][enabled]`]);
        console.log(`Cottage ${updateCottageIndex} adultCount:`, req.body[`cottages[${updateCottageIndex}][includedEntranceFee][adultCount]`]);
        console.log(`Cottage ${updateCottageIndex} childCount:`, req.body[`cottages[${updateCottageIndex}][includedEntranceFee][childCount]`]);
        
        // Check all keys that contain includedEntranceFee for this cottage
        const allCottageKeys = Object.keys(req.body).filter(key => key.includes(`cottages[${updateCottageIndex}]`) && key.includes('includedEntranceFee'));
        console.log(`All includedEntranceFee keys for Cottage ${updateCottageIndex}:`, allCottageKeys);
        
        const cottageIncludedEntranceFee = {
          enabled: req.body[`cottages[${updateCottageIndex}][includedEntranceFee][enabled]`] === "true" || 
                   req.body[`cottages[${updateCottageIndex}][includedEntranceFee][enabled]`] === true,
        ...
        console.log(`Cottage ${updateCottageIndex} parsed includedEntranceFee:`, cottageIncludedEntranceFee);
        ...
        cottages.push({
          ...
          includedEntranceFee: cottageIncludedEntranceFee,
        });
        updateCottageIndex++;
      }
      if (cottages.length > 0) {
