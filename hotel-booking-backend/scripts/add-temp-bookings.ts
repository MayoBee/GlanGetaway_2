...
async function addTempBookings() {
  try {
    ...
    await db.collection('bookings').deleteMany({ userId: user._id });
    ...
    bookings.forEach((booking, index) => {
      ...
      console.log(`${index + 1}. ${hotel?.name || 'Unknown Hotel'} - ${booking.status} - ${booking.checkIn.toDateString()} to ${booking.checkOut.toDateString()} - ₱${booking.totalCost}`);
    });
...
export default addTempBookings;
